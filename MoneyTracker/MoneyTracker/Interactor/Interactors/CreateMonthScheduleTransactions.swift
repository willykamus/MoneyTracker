        ...
        for scheduledTransaction in scheduledTransactions {
            ...
            case .day:
                for i in 0..<Calendar.current.numberOfDaysBetween(from: date, to: Date().endOfMonth()) {
                    let newTransaction = Transaction(id: "", amount: scheduledTransaction.transaction.amount, category: scheduledTransaction.transaction.category, date: Calendar.current.date(byAdding: .day, value: i + 1, to: date)!, containerId: scheduledTransaction.transaction.containerId, containerName: scheduledTransaction.transaction.containerName, type: scheduledTransaction.transaction.type)
            ...
            case .twoWeeks:
                ...
                if (remainingWeeks / 2) >= 1 {
                    for i in 0..<(remainingWeeks / 2) {
                        let newTransaction = Transaction(id: UUID().uuidString, amount: scheduledTransaction.transaction.amount, category: scheduledTransaction.transaction.category, date: Calendar.current.date(byAdding: .weekOfMonth, value: i + 2, to: date)!, containerId: scheduledTransaction.transaction.containerId, containerName: scheduledTransaction.transaction.containerName, type: scheduledTransaction.transaction.type)
                        let scheduled = ScheduledTransaction(id: UUID().uuidString, transaction: newTransaction, recurrence: scheduledTransaction.recurrence)
                        newTransactions.append(scheduled)
                    }
                }
